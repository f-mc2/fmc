...
include-in-header:
  text: |
    ...
    <style>
      .ggb-wrap{
        position:relative;width:100%;max-width:960px;margin:1rem auto;
        border-radius:12px;overflow:hidden;box-shadow:0 8px 28px rgba(0,0,0,.12);
        background:var(--quarto-body-bg,#fff);
      }
      /* a bit taller so the input bar fits comfortably */
      .ggb-16x10{ padding-top: 62.5%; }
      .ggb-canvas{ position:absolute; inset:0; width:100%; height:100%; }
    </style>
---
...
Let $f\colon X\rightarrow Y$ be a function. The notion of limit formalizes, rigorously, what happens to the output when the inputs gets closer and closer to a fixed value. Roughly speaking, we write
...
Below, there is an interactive visualization of the $(\varepsilon-\delta)$-definition of the limit of a function when everything works (source: [Jason McCullough in Geogebra](https://www.geogebra.org/m/mj2bXA5y)).
The function used is $f\colon\mathbb{R}\rightarrow\mathbb{R}$ given by $f(x) = 1/8 x^3$, but can be changed.
The sliders can be used to change the value of $\varepsilon$ and $\delta$ (which corresponds to $\delta_{\varepsilon}$ in @def-limits).
The reference point can be dragged to explore the behaviour at other points in the domain of the function.
The demo assumes the function to be continuous and monotone in the interval $(x - \delta, x + \delta)$.


<div class="ggb-wrap ggb-16x10">
  <div id="ggb-container" class="ggb-canvas"></div>
</div>

<script>
window.addEventListener('DOMContentLoaded', () => {
  const params = {
    material_id: "mj2bXA5y",

    // Use Classic to avoid command-compat issues (e.g., Polygon)
    appName: "classic",

    // Keep UI minimal but show the input bar like the original
    showToolBar: false,
    showMenuBar: false,
    showAlgebraInput: true,
    algebraInputPosition: "bottom", // works with Classic
    showZoomButtons: true,
    enableRightClick: false,
    allowStyleBar: false,
    language: "en",

    // Make it fill the wrapper responsively
    scaleContainerClass: "ggb-wrap",
    autoHeight: false,
    allowUpscale: true,

    // Required by loader; CSS controls final size
    width: 800, height: 500,

    // Optional: tidy initial view once loaded
    appletOnLoad: (api) => {
      try { api.setCoordSystem(-6, 6, -6, 6); } catch(_) {}
    }
  };

  const applet = new GGBApplet(params, /* useBrowserForJS */ true);
  applet.inject("ggb-container");
});
</script>


Below, there is an interactive visualization of the $(\varepsilon-\delta)$-definition of the limit of a function when things do not work ([Jason McCullough in Geogebra](https://www.geogebra.org/m/hebkhRJm)).
Note that a trignometric function is used even if they are introduced later):
