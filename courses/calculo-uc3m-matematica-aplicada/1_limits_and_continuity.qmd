...
The function used in the applet is $f\colon\mathbb{R}\rightarrow\mathbb{R}$ with $f(x) = 1/8 x^3$, but can be changed in the input bar.
...
$$
as follows.
The interval $(L-\varepsilon, L+\varepsilon)$ in the output space is depicted as an horizontal blue strip, while the interval $(a-\delta,a+\delta)$ in the input space is depicted as a vertical red strip, and we can use the sliders to change the value of $\varepsilon>0$ and $\delta>0$.  
What happens is that, whatever the value of $\varepsilon$ is, we can find a suitable value for $\delta$ such that $x\in (a-\delta,a+\delta)$ (or, equivalently, $|x-a|<\delta$) implies $f(x)\in (L-\varepsilon, L+\varepsilon)$ (or, equivalently, $|f(x)- L|<\epsilon$.
...
The first thing to do once limits are defined as in @def-limits is to prove that they are indeed well-defined in the sense that a limit, if it exists, is unique.
...
In @def-limits, three types of limits for $x$ going to $x_{0}$ are considered, and in the exercise below, you are asked to take some time to prove that, under appropriate conditions, they are equivalent in a precise sense.
...
As we stressed [before](0_sets_and_functions.qmd), a rigorous definition of a function requires a domain to be specified.
...
$$
The following exercise asks you to pause and prove that this weird behavior is not compatible with @def-limits.
...
# Three simple examples {#sec-limits-constant-linear-quadratic-functions}
...
This difficulty bothered some mathematicians who decided to investigate some simple cases using _educated guesses_ (that is, the sophisticated siblings of sheer luck), and then proceeded to prove general theorems that allows to exploit to the maximum the simple examples already solved.
